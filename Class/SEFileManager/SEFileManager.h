...
typedef NS_ENUM(NSInteger, SESemanticAnnounce) {
    SESemanticAnnounceUntype = 0,    //无类型
    SESemanticAnnounceText   = 1,    //文字
    SESemanticAnnounceImage  = 2,    //图片 或 图片+文字
    SESemanticAnnounceVideo  = 3,    //视频
    SESemanticAnnounceAudio  = 4,    //音频
    SESemanticAnnouncePDF    = 5,    //PDF
    SESemanticAnnounceOther          //不识别
};

typedef NS_ENUM(NSInteger, SESpaceAnnounce) {
    SESpaceAnnounceFile  = 0,   //文件
    SESpaceAnnounceImage = 1    //图片
};

typedef void(^UploadSuccessBlock)(NSString *url);
typedef void(^UploadFailureBlock)(NSError *error);

@interface SEFileManager : NSObject
