...
{
    return [[SEFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[SEFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[SEFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[SEFileManager pathForLibraryDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[SEFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[SEFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
...
+ (void)uploadFileAtSpaceAnnounce:(SESpaceAnnounce)spaceAnnounce
                          ...
                          failure:(UploadFailureBlock)failure
{
    ...
    NSDictionary *parameters = @{@"key":fileName,@"type":@(spaceAnnounce)};
    
    [[SEAccountNetworking sharedInstance] getQiniuTokenAtParameters:parameters success:^(NSURLSessionDataTask *task, SEResponseObject *responseObject) {
        
        if (responseObject.httpCode == SEResponseHTTPCode200) {
            NSString *url = responseObject.result[@"key"];
            NSString *uploadToken = responseObject.result[@"token"];
            
            [[[QNUploadManager alloc] init] putData:data key:fileName token:uploadToken complete:^(QNResponseInfo *info, NSString *oldkey, NSDictionary *resp) {
                
                if (info.ok) {
                    success(url.mutableCopy);
                } else {
                    success(nil);
                }
            } option:nil];
            ...
            if (failure) {
                failure([NSError errorWithDomain:responseObject.msg code:responseObject.httpCode userInfo:[responseObject modelToJSONObject]]);
            }
        }
