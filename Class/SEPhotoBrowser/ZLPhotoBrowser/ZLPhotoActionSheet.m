...
@property (weak, nonatomic) IBOutlet ShowSuccessBlock showSuccessBlock;
...
- (void)layoutSubviews
{
    ...
    if (!self.configuration.allowSelectImage && self.configuration.allowRecordVideo) {
        [self.cameraButton setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCameraRecordText) forState:UIControlStateNormal];
    } else {
        [self.cameraButton setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCameraText) forState:UIControlStateNormal];
    }
    [self.photoButton setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserAblumText) forState:UIControlStateNormal];
    [self.cancelButton setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCancelText) forState:UIControlStateNormal];
...
- (void)showPreviewAnimated:(BOOL)animate callBack:(ShowSuccessBlock)callBack
{
    if (callBack) {
        self.showSuccessBlock = callBack;
    }
    [self showPreview:YES animate:animate];
}
...
- (void)previewSelectedPhotos:(NSArray<UIImage *> *)photos assets:(NSArray<PHAsset *> *)assets index:(NSInteger)index isOriginal:(BOOL)isOriginal
{
    ...
    svc.previewSelectedImageBlock = ^(NSArray<UIImage *> *arrP, NSArray<PHAsset *> *arrA) {
        ...
        NSLog(@"选中：%@",arrP);
        if (strongSelf.selectPhotoImageBlock) {
            strongSelf.selectPhotoImageBlock(arrP, arrA, NO);
        }
        [strongSelf hide];
...
#pragma mark - 拍照
- (IBAction)camera_Click_Action:(id)sender
{
...
#pragma mark - 相册
- (IBAction)photo_Click_Action:(id)sender
{
    if (![ZLPhotoManager havePhotoLibraryAuthority]) {
        //无游览权限
        [self showNoAuthorityVC];
...
- (IBAction)cancel_Click_Action:(id)sender
{
...
- (void)changeCancelBtnTitle
{
    if (self.arrSelectedModels.count > 0) {
        [self.cancelButton setTitle:[NSString stringWithFormat:@"%@(%ld)", GetLocalLanguageTextValue(ZLPhotoBrowserDoneText), self.arrSelectedModels.count] forState:UIControlStateNormal];
        [self.cancelButton setTitleColor:self.configuration.bottomBtnsNormalTitleColor forState:UIControlStateNormal];
    } else {
        [self.cancelButton setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCancelText) forState:UIControlStateNormal];
...
- (void)requestSelPhotos:(UIViewController *)vc data:(NSArray<ZLPhotoModel *> *)data hideAfterCallBack:(BOOL)hide
{
    ...
    if (!self.configuration.shouldAnialysisAsset) {
        ...
        [hud hide];
        if (self.selectPhotoImageBlock) {
            self.selectPhotoImageBlock(nil, assets, self.isSelectOriginalPhoto);
            [self.arrSelectedModels removeAllObjects];
        }
        if (hide) {
            [self hide];
    ...
    for (int i = 0; i < data.count; i++) {
        ...
        [ZLPhotoManager requestSelectedImageForAsset:model isOriginal:self.isSelectOriginalPhoto allowSelectGif:self.configuration.allowSelectGif completion:^(UIImage *image, NSDictionary *info) {
            ...
            [hud hide];
            if (strongSelf.selectPhotoImageBlock) {
                strongSelf.selectPhotoImageBlock(photos, assets, strongSelf.isSelectOriginalPhoto);
...
- (ZLImageNavigationController *)getImageNavWithRootVC:(UIViewController *)rootVC
{
    ...
    [nav setCallSelectClipImageBlock:^(UIImage *image, PHAsset *asset){
        zl_strongify(weakSelf);
        if (strongSelf.selectPhotoImageBlock) {
            strongSelf.selectPhotoImageBlock(@[image], @[asset], NO);
        }
